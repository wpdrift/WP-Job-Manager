	...
	<?php if ( job_manager_user_can_post_job() || job_manager_user_can_edit_job( $job_id ) ) : ?>
		...
		<?php foreach ( $job_fields as $key => $field ) : ?>
			<fieldset class="fieldset-<?php echo esc_attr( $key ); ?>">
				<label for="<?php echo esc_attr( $key ); ?>"><?php echo $field['label'] . apply_filters( 'submit_job_form_required_label', $field['required'] ? '' : ' <small>' . __( '(optional)', 'wp-restaurant-listings' ) . '</small>', $field ); ?></label>
		...
		<?php if ( $company_fields ) : ?>
			<h2><?php _e( 'Company Details', 'wp-restaurant-listings' ); ?></h2>
